    ...
    for rrset in response.authority:
        if rrset.rdtype == dns.rdatatype.NS:
            for rr in rrset:
                ...
                print(f"[INFO] Extracted NS hostname: {ns_name}")
    ...
    for ns_name in ns_names:
        try:
            ...
            for rdata in answers:
                ns_ip = rdata.to_text()
                ns_ips.append(ns_ip)
                print(f"[INFO] Resolved {ns_name} to {ns_ip}")
...
def iterative_dns_lookup(domain):
    ...
    """
    print(f"[START] Iterative DNS lookup for {domain}")
    ...
    while next_ns_list:
        ...
        if response:
            print(f"[DEBUG] Query to {stage} server {ns_ip} succeeded")

            # Check if answer section contains A record
            if response.answer:
                for answer in response.answer:
                    if answer.rdtype == dns.rdatatype.A:
                        print(f"[SUCCESS] {domain} resolved to {answer[0].to_text()}")
                        return
...
def recursive_dns_lookup(domain):
    ...
    Args:
        domain (str): Domain name to resolve.
    """
    print(f"[START] Recursive DNS lookup for {domain}")
    try:
        resolver = dns.resolver.Resolver()
        answers = resolver.resolve(domain, "A")
        for rdata in answers:
            print(f"[SUCCESS] {domain} resolved to {rdata.to_text()}")
    except dns.resolver.NXDOMAIN:
        print(f"[ERROR] Domain {domain} does not exist (NXDOMAIN)")
    except dns.resolver.NoAnswer:
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Time taken: {time.time() - start_time:.3f} seconds")